...
******************************************************************************/
static const int GENCMD_MAX_LENGTH = 512;
...
gencmd_client = ffi.new("GENCMD_SERVICE_T");


/******************************************************************************
Static function.
******************************************************************************/
static void gencmd_callback( void *callback_param, VCHI_CALLBACK_REASON_T reason, void *msg_handle );

function lock_obtain ()
   local ret = -1;
   
   if(gencmd_client.initialised and vcos_mutex_lock(&gencmd_client.lock) == VCOS_SUCCESS) then
      ret = 0;
   end
...
function use_gencmd_service() 
   ...
   local i=0;
   while (i < gencmd_client.num_connections) do
      if ret == 0 then
...
function release_gencmd_service() 
   ...
   int ret = 0;
   local i = 0;
   while (i < gencmd_client.num_connections) 
...
local READING_PROPERTY = 0
local READING_VALUE = 1
local READING_VALUE_QUOTED = 2
